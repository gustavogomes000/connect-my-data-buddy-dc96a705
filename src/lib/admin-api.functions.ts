...
export const deleteProgramacao = createAdminServerFn("POST")
  ...
  .handler(async ({ data }) => {
      const supabase = await getAdminSupabase();
    const { error } = await supabase.from("programacao").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { success: true };
  });
...
export const deletePodcast = createAdminServerFn("POST")
  ...
  .handler(async ({ data }) => {
      const supabase = await getAdminSupabase();
    const { error } = await supabase.from("podcasts").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { success: true };
  });
...
export const deletePromotionEntry = createAdminServerFn("POST")
  ...
  .handler(async ({ data }) => {
      const supabase = await getAdminSupabase();
    const { error } = await supabase.from("promotion_entries").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { success: true };
  });
